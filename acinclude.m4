...
AC_ARG_ENABLE(wx-shared, [  --disable-wx-shared    statically link wxWidgets],,enable_wx_shared="yes")
  AC_MSG_CHECKING(linking wxWidgets as shared lib)
  ...
  if test "$WX_CONFIG" = "no" ; then
    ...
    if test x$enable_wx_shared = "xyes" ; then
       ...
       WX_LIBS=`$WX_CONFIG $wx_config_args --static=yes --libs`
    fi
