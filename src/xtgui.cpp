...
char **XtGui::pixmapname[8] = { floppy0, floppy1, floppy2,
...
/*ARGSUSED*/
void buttonPress(Widget w, XEvent *pevent, String *pparams, Cardinal *pnparams)
{
	ggui->c_buttonPress(pevent);
}

/*ARGSUSED*/
void buttonRelease(Widget w, XEvent *pevent, String *pparams, Cardinal *pnparams)
{
	ggui->c_buttonRelease(pevent);
}

/*ARGSUSED*/
void enter(Widget w, XEvent *pevent, String *pparams, Cardinal *pnparams)
{
	ggui->c_enter(pevent);
}

/*ARGSUSED*/
void leave(Widget w, XEvent *pevent, String *pparams, Cardinal *pnparams)
{
	ggui->c_leave(pevent);
}

/*ARGSUSED*/
void motion(Widget w, XEvent *pevent, String *pparams, Cardinal *pnparams)
{
...
/*ARGSUSED*/
void focusOutMain(Widget w, XEvent *pevent, String *pparams, Cardinal *pnparams)
{
...
/*ARGSUSED*/
void wm_protocols(Widget w, XEvent *pevent, String *pparams, Cardinal *pnparams)
{
...
/*ARGSUSED*/
void XtGui::toggleUndocumentedCallback(Widget w,
	XtPointer client_data, XtPointer call_data)
{
	if (client_data != NULL)
...
/*ARGSUSED*/
void XtGui::toggleFrequencyCallback(Widget w,
	XtPointer client_data, XtPointer call_data)
{
	if (client_data != NULL)
...
/*ARGSUSED*/
void XtGui::popupBpCallback(Widget w,
	XtPointer client_data, XtPointer call_data)
{
	if (client_data != NULL)
...
/*ARGSUSED*/
void XtGui::popupLogfileCallback(Widget w,
	XtPointer client_data, XtPointer call_data)
{
	if (client_data != NULL)
...
/*ARGSUSED*/
void XtGui::clearLogCallback(Widget w,
	XtPointer client_data, XtPointer call_data)
{
	if (client_data != NULL)
...
void XtGui::create_pixmaps(Widget w, Pixel color)
{
	...
	for (i = PM_FLOPPY0; i <= PM_IRQ4; ++i)
	{
		...
		errorStatus = XpmCreatePixmapFromData(XtDisplay(w),
			XtWindow(w), pixmapname[i], &pixmap[i], NULL, &xpmattr);
...
void XtGui::popup_about(void)
{
	...
	XtPopup(aboutframe, XtGrabNonexclusive);
			...
			int width, int height)
{
	...
	XtPopup(messageframe, XtGrabNone);
	XtAddGrab(messageframe, True, False);
}
...
void XtGui::popup_interrupt_info(Widget w)
{
        if (schedy == NULL)
                return;
...
void XtGui::update_disk_status(int floppyIndex, tDiskStatus status)
{
	...
	XtVaSetValues(floppybutton[floppyIndex], XtNbitmap,
        ...
        switch(status)
        {
        case DISK_STAT_EMPTY:    label = " "; break;
        case DISK_STAT_INACTIVE: label = "O"; break;
        case DISK_STAT_ACTIVE:   label = "X"; break;
        }
...
void XtGui::update_interrupt_status(tIrqType irqType, bool status)
{
	...
	if (status)
	{
                switch(irqType)
        	{
        	case INT_IRQ:    label = "I"; break;
        	case INT_FIRQ:   label = "F"; break;
        	case INT_NMI:    label = "N"; break;
        	case INT_RESET:  label = "R"; break;
        	}
	}
...
void XtGui::timerCallback(XtIntervalId id)
{
...
void XtGui::c_buttonPress(XEvent *pevent)
{
	...
	{
	case Button1: mouse_button_state |= Button1Mask; break;
	case Button2: mouse_button_state |= Button2Mask; break;
	case Button3: mouse_button_state |= Button3Mask; break;
	case Button4: mouse_button_state |= Button4Mask; break;
	case Button5: mouse_button_state |= Button5Mask; break;
	}
} // c_buttonPress
...
Widget XtGui::create_main_view(int argc, char *argv[], int synchronized)
{
	...
	static XtActionsRec	actions[] = {
					...
					{"wm_protocols", (XtActionProc)wm_protocols}
				};
...
void XtGui::initialize_after_create(Widget w, int inverse, const char *color)
{
	...
	for (i = 0; i < MAX_GUIXSIZE; i++) {
		for (j = 0; j < MAX_GUIYSIZE; j++) {
			...
			switch (depth) {
			  case  1:
			  case  8: image6[i][j]->bitmap_unit = 8; break;
			  case 15:
			  case 16: image6[i][j]->bitmap_unit = 16; break;
			  case 24:
			  case 32: image6[i][j]->bitmap_unit = 32; break;
			}
	...
	XtAddCallback(cpubutton,   XtNcallback, popdownCpuCallback,
       ...
       cpupixmap = None;
        ...
        errorStatus = XpmCreatePixmapFromData(XtDisplay(cpuframe),
                XtWindow(cpuframe), flexcpu, &cpupixmap,
                NULL, &xpmattr);
        XpmFreeAttributes(&xpmattr);
        if (errorStatus != XpmSuccess)
#endif
...
void XtGui::redraw_cpuview_impl(const Mc6809CpuStatus &stat)
{
	...
	if (stat.state == S_RESET_RUN)
		XawToggleSetCurrent(runbutton, (XtPointer)&radio_data[S_RESET]);
	else if (stat.state == S_INVALID)
		XawToggleSetCurrent(runbutton, (XtPointer)&radio_data[S_STOP]);
	else
