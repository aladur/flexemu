    ...
    , is_flex_format(true)
{
        ...
        flx::fromBigEndian(flx_header.magic_number) == MAGIC_NUMBER)
    {
        ...
        {
            // This is a FLX disk image file but it is not compatible
            // to FLEX.
            is_flex_format = false;
        }
        return;
                            ...
                            int side, unsigned sizecode)
{
    ...
    {
        // For FLX format update file header.
        fstream.seekg(0);
        if (fstream.fail())
        {
            result = false;
    ...
    const auto estimatedFileSize = getFileSize(flx_header) -
        (param.type == DiskType::IMA ? sizeof(s_flex_header) : 0);
    if (result && !is_flex_format && (file_size == estimatedFileSize) &&
        IsFlexFileFormat(param.type))
    {
        is_flex_format = true;
    }
...
{
    // An unformatted disk can have IMA or FLX format.
    // If file extension is not *.ima FLX format is used as default.
    const auto extension = flx::tolower(path.extension().u8string());
    const bool isFlxDisk = (extension != ".ima");

    file_size = isFlxDisk ? sizeof(struct s_flex_header) : 0U;
    param = { };
    param.offset = isFlxDisk ? sizeof(struct s_flex_header) : 0U;
    param.type = isFlxDisk ? DiskType::FLX : DiskType::IMA;
...
// return != 0 on success
// format FLX, DSK or IMA format. FLX format always with sector_size 256
