 ...
 flaginstr:  /* $10 and $11 instructions return here */
  ...
  if(haspostbyte[ireg]) {
   ...
   switch(postbyte) {
    ...
    case 0x97: eaddr=0; INVALID_POST break; /*ILELGAL*/
    ...
    case 0x9A: eaddr=0; INVALID_POST break; /*ILLEGAL*/ 
    ...
    case 0x9E: eaddr=0; INVALID_POST break; /*ILLEGAL*/   
    ...
    case 0xAE: eaddr=0; INVALID_POST break; /*ILLEGAL*/   
    ...
    case 0xBE: eaddr=0; INVALID_POST break; /*ILLEGAL*/   
    ...
    case 0xCE: eaddr=0; INVALID_POST break; /*ILLEGAL*/   
    ...
    case 0xDE: eaddr=0; INVALID_POST break; /*ILLEGAL*/   
    ...
    case 0xEE: eaddr=0; INVALID_POST break; /*ILLEGAL*/   
    ...
    case 0xFE: eaddr=0; INVALID_POST break; /*ILLEGAL*/   
    case 0xFF: IMMWORD(eaddr);eaddr=GETWORD(eaddr);break;
   } 
  }
  switch(ireg) {
   case 0x00: /*NEG direct*/ DIRECT k=GETBYTE(eaddr); tw=-k;
   		             SETSTATUS(0,k,tw)
                             SETBYTE(eaddr,tw) break;
   case 0x01: INVALID_INSTR break;/*ILLEGAL*/
   case 0x02: INVALID_INSTR break;/*ILLEGAL*/
   case 0x03: /*COM direct*/ DIRECT  tb=~GETBYTE(eaddr);SETNZ8(tb);SEC CLV
                             SETBYTE(eaddr,tb); break;
   case 0x04: /*LSR direct*/ DIRECT tb=GETBYTE(eaddr);if(tb&0x01)SEC else CLC
                             if(tb&0x10)SEH else CLH tb>>=1;SETNZ8(tb)
                             SETBYTE(eaddr,tb)break;
   case 0x05: INVALID_INSTR break;/* ILLEGAL*/
   case 0x06: /*ROR direct*/ DIRECT tb=(iccreg&0x01)<<7;
                             k=GETBYTE(eaddr);
                              ...
                              SETBYTE(eaddr,tw)
                       	     break;
   case 0x07: /*ASR direct*/ DIRECT tb=GETBYTE(eaddr);if(tb&0x01)SEC else CLC
                             if(tb&0x10)SEH else CLH tb>>=1;
                             if(tb&0x40)tb|=0x80;SETBYTE(eaddr,tb) SETNZ8(tb)
                             break;
   case 0x08: /*ASL direct*/ k=GETBYTE(eaddr);
   			     DIRECT tw=k<<1;
                             SETSTATUS(k,k,tw)
                             SETBYTE(eaddr,tw) break;
   case 0x09: /*ROL direct*/ DIRECT tb=GETBYTE(eaddr);tw=iccreg&0x01;
                             if(tb&0x80)SEC else CLC
                             if((tb&0x80)^((tb<<1)&0x80))SEV else CLV
                             tb=(tb<<1)+tw;SETNZ8(tb) SETBYTE(eaddr,tb)break;
   case 0x0A: /*DEC direct*/ DIRECT tb=GETBYTE(eaddr)-1;if(tb==0x7F)SEV else CLV
   			     SETNZ8(tb) SETBYTE(eaddr,tb)break;
   case 0x0B: INVALID_INSTR break; /*ILLEGAL*/
   case 0x0C: /*INC direct*/ DIRECT tb=GETBYTE(eaddr)+1;if(tb==0x80)SEV else CLV
   			     SETNZ8(tb) SETBYTE(eaddr,tb)break;   			                                  
   case 0x0D: /*TST direct*/ DIRECT tb=GETBYTE(eaddr);SETNZ8(tb) break;
   ...
   case 0x12: /* NOP */ break;
#ifdef FASTFLEX
   case 0x13: /* SYNC */ ipcreg--; events |= DO_SYNC; break;
#else
...
#endif
		         break;
   case 0x14: INVALID_INSTR break; /*ILLEGAL*/
   case 0x15: INVALID_INSTR break; /*ILLEGAL*/
   case 0x16: /*LBRA*/ IMMWORD(eaddr) ipcreg+=eaddr;break;
   case 0x17: /*LBSR*/ IMMWORD(eaddr) PUSHWORD(ipcreg) ipcreg+=eaddr;break;
   case 0x18: INVALID_INSTR break; /*ILLEGAL*/
   case 0x19: /* DAA*/ 	tw=iareg;
			if(iccreg&0x20 || (iareg&0x0f)>9)tw+=6;
 			if(iccreg&0x01 || (iareg&0xf0)>0x90 ||
			   ((iareg&0xf0)>0x80 && (iareg&0x0f)>0x09))tw+=0x60;
			if(tw&0x100)SEC
   ...
   case 0x1B: INVALID_INSTR break; /*ILLEGAL*/
   ...
   case 0x3E: INVALID_INSTR break; /*ILLEGAL*/                    
   ...
   case 0x40: /*NEGA*/  tw=-iareg;SETSTATUS(0,iareg,tw)
                             iareg=(Byte)tw;break;
   case 0x41: INVALID_INSTR break;/*ILLEGAL*/
   case 0x42: INVALID_INSTR break;/*ILLEGAL*/
   case 0x43: /*COMA*/   tb=~iareg;SETNZ8(tb);SEC CLV
                             iareg=tb;break;
   case 0x44: /*LSRA*/  tb=iareg;if(tb&0x01)SEC else CLC
                             if(tb&0x10)SEH else CLH tb>>=1;SETNZ8(tb)
                             iareg=tb;break;
   case 0x45: INVALID_INSTR break;/* ILLEGAL*/
   ...
   case 0x5B: INVALID_INSTR break; /*ILLEGAL*/
   case 0x5C: /*INCB*/  tb=ibreg+1;if(tb==0x80)SEV else CLV
   			     SETNZ8(tb) ibreg=tb;break;   			                                  
   case 0x5D: /*TSTB*/  SETNZ8(ibreg) break;
   case 0x5E: INVALID_INSTR break; /*ILLEGAL*/
   case 0x5F: /*CLRB*/  ibreg=0;CLN CLV SEZ CLC break;
   case 0x60: /*NEG indexed*/  k=GETBYTE(eaddr);tw=-k;
                             SETSTATUS(0,k,tw)
                             SETBYTE(eaddr,tw) break;
   case 0x61: INVALID_INSTR break;/*ILLEGAL*/
   case 0x62: INVALID_INSTR break;/*ILLEGAL*/
   case 0x63: /*COM indexed*/   tb=~GETBYTE(eaddr);SETNZ8(tb);SEC CLV
                             SETBYTE(eaddr,tb)break;
   case 0x64: /*LSR indexed*/  tb=GETBYTE(eaddr);if(tb&0x01)SEC else CLC
                             if(tb&0x10)SEH else CLH tb>>=1;SETNZ8(tb)
                             SETBYTE(eaddr,tb)break;
   case 0x65: INVALID_INSTR break;/* ILLEGAL*/
   case 0x66: /*ROR indexed*/  tb=(iccreg&0x01)<<7;
                             k=GETBYTE(eaddr); if(k&0x01)SEC else CLC
                             tw=(k>>1)+tb;SETNZ8(tw)
                             SETBYTE(eaddr,tw)
                       	     break;
   case 0x67: /*ASR indexed*/  tb=GETBYTE(eaddr);if(tb&0x01)SEC else CLC
                             if(tb&0x10)SEH else CLH tb>>=1;
                             if(tb&0x40)tb|=0x80;SETBYTE(eaddr,tb)SETNZ8(tb)
                             break;
   case 0x68: /*ASL indexed*/k=GETBYTE(eaddr); tw=k<<1;
                             SETSTATUS(k,k,tw)
                             SETBYTE(eaddr,tw) break;
   case 0x69: /*ROL indexed*/  tb=GETBYTE(eaddr);tw=iccreg&0x01;
                             if(tb&0x80)SEC else CLC
                             if((tb&0x80)^((tb<<1)&0x80))SEV else CLV
                             tb=(tb<<1)+tw;SETNZ8(tb) SETBYTE(eaddr,tb)break;
   case 0x6A: /*DEC indexed*/  tb=GETBYTE(eaddr)-1;if(tb==0x7F)SEV else CLV
   			     SETNZ8(tb) SETBYTE(eaddr,tb)break;
   case 0x6B: INVALID_INSTR break; /*ILLEGAL*/
   case 0x6C: /*INC indexed*/  tb=GETBYTE(eaddr)+1;if(tb==0x80)SEV else CLV
   			     SETNZ8(tb) SETBYTE(eaddr,tb)break;   			                                  
   case 0x6D: /*TST indexed*/  tb=GETBYTE(eaddr);SETNZ8(tb) break;
   case 0x6E: /*JMP indexed*/  ipcreg=eaddr;break;
   case 0x6F: /*CLR indexed*/  SETBYTE(eaddr,0)CLN CLV SEZ CLC break;
   case 0x70: /*NEG ext*/ EXTENDED k=GETBYTE(eaddr); tw=-k;
                             SETSTATUS(0,k,tw)
                             SETBYTE(eaddr,tw)break;
   case 0x71: INVALID_INSTR break;/*ILLEGAL*/
   case 0x72: INVALID_INSTR break;/*ILLEGAL*/
   case 0x73: /*COM ext*/ EXTENDED  tb=~GETBYTE(eaddr);SETNZ8(tb);SEC CLV
                            SETBYTE(eaddr,tb)break;
   case 0x74: /*LSR ext*/ EXTENDED tb=GETBYTE(eaddr);if(tb&0x01)SEC else CLC
                             if(tb&0x10)SEH else CLH tb>>=1;SETNZ8(tb)
                             SETBYTE(eaddr,tb)break;
   case 0x75: INVALID_INSTR break;/* ILLEGAL*/
   case 0x76: /*ROR ext*/ EXTENDED tb=(iccreg&0x01)<<7;
                             k=GETBYTE(eaddr); if(k&0x01)SEC else CLC
                             tw=(k>>1)+tb;SETNZ8(tw)
                             SETBYTE(eaddr,tw)
                       	     break;
   case 0x77: /*ASR ext*/ EXTENDED tb=GETBYTE(eaddr);if(tb&0x01)SEC else CLC
                             if(tb&0x10)SEH else CLH tb>>=1;
                             if(tb&0x40)tb|=0x80;SETBYTE(eaddr,tb)SETNZ8(tb)
                             break;
   case 0x78: /*ASL ext*/ EXTENDED k=GETBYTE(eaddr); tw=k<<1;
                             SETSTATUS(k,k,tw)
                             SETBYTE(eaddr,tw)break;
   case 0x79: /*ROL ext*/ EXTENDED tb=GETBYTE(eaddr);tw=iccreg&0x01;
                             if(tb&0x80)SEC else CLC
                             if((tb&0x80)^((tb<<1)&0x80))SEV else CLV
                             tb=(tb<<1)+tw;SETNZ8(tb) SETBYTE(eaddr,tb)break;
   case 0x7A: /*DEC ext*/ EXTENDED tb=GETBYTE(eaddr)-1;if(tb==0x7F)SEV else CLV
   			     SETNZ8(tb) SETBYTE(eaddr,tb)break;
   case 0x7B: INVALID_INSTR break; /*ILLEGAL*/
   case 0x7C: /*INC ext*/ EXTENDED tb=GETBYTE(eaddr)+1;if(tb==0x80)SEV else CLV
   			     SETNZ8(tb) SETBYTE(eaddr,tb)break;   			                                  
   case 0x7D: /*TST ext*/ EXTENDED tb=GETBYTE(eaddr);SETNZ8(tb) break;
   case 0x7E: /*JMP ext*/ EXTENDED ipcreg=eaddr;break;
   case 0x7F: /*CLR ext*/ EXTENDED SETBYTE(eaddr,0)CLN CLV SEZ CLC break;
   case 0x80: /*SUBA immediate*/ IMM8 k=GETBYTE(eaddr);
                                 tw=iareg-k;
                                 SETSTATUS(iareg,k,tw)
                                 iareg=(Byte)tw;break;
   case 0x81: /*CMPA immediate*/ IMM8 k=GETBYTE(eaddr);
                                 tw=iareg-k;
   				 SETSTATUS(iareg,k,tw) break;
   case 0x82: /*SBCA immediate*/ IMM8 k=GETBYTE(eaddr);
   ...
   case 0x84: /*ANDA immediate*/ IMM8 iareg=iareg&GETBYTE(eaddr);SETNZ8(iareg)
   				 CLV break;
   case 0x85: /*BITA immediate*/ IMM8 tb=iareg&GETBYTE(eaddr);SETNZ8(tb)
   				 CLV break;
   ...
   case 0x88: /*EORA immediate*/ IMM8 iareg=iareg^GETBYTE(eaddr);SETNZ8(iareg)
   				 CLV break;
   case 0x89: /*ADCA immediate*/ IMM8 k=GETBYTE(eaddr);
                                 tw=iareg+k+(iccreg&0x01);
                                 SETSTATUS(iareg,k,tw)
                                 iareg=(Byte)tw;break;   				 
   case 0x8A: /*ORA immediate*/  IMM8 iareg=iareg|GETBYTE(eaddr);SETNZ8(iareg)
   				 CLV break;
   case 0x8B: /*ADDA immediate*/ IMM8 k=GETBYTE(eaddr);
                                 tw=iareg+k;
   				 SETSTATUS(iareg,k,tw)
   				 iareg=(Byte)tw;break;
   ...
   case 0x90: /*SUBA direct*/ DIRECT k=GETBYTE(eaddr); tw=iareg-k;
                                 SETSTATUS(iareg,k,tw)
                                 iareg=(Byte)tw;break;
   case 0x91: /*CMPA direct*/ DIRECT k=GETBYTE(eaddr); tw=iareg-k;
   				 SETSTATUS(iareg,k,tw)  break;
   case 0x92: /*SBCA direct*/ DIRECT k=GETBYTE(eaddr);
                                 tw=iareg-k-(iccreg&0x01);
   				 SETSTATUS(iareg,k,tw)
   				 iareg=(Byte)tw;break;
   ...
   case 0x94: /*ANDA direct*/ DIRECT iareg=iareg&GETBYTE(eaddr);
                                 SETNZ8(iareg)
   				 CLV break;
   case 0x95: /*BITA direct*/ DIRECT tb=iareg&GETBYTE(eaddr);SETNZ8(tb)
   				 CLV break;
   case 0x96: /*LDA direct*/ DIRECT LOADAC(iareg) CLV SETNZ8(iareg)
                                 break;
   case 0x97: /*STA direct */ DIRECT
                                 SETNZ8(iareg) CLV STOREAC(iareg) break;
   case 0x98: /*EORA direct*/ DIRECT iareg=iareg^GETBYTE(eaddr);
                                 SETNZ8(iareg)
   				 CLV break;
   case 0x99: /*ADCA direct*/ DIRECT k=GETBYTE(eaddr);
                                 tw=iareg+k+(iccreg&0x01);
                                 SETSTATUS(iareg,k,tw)
                                 iareg=(Byte)tw;break;   				 
   case 0x9A: /*ORA direct*/  DIRECT iareg=iareg|GETBYTE(eaddr);
                                 SETNZ8(iareg)
   				 CLV break;
   case 0x9B: /*ADDA direct*/ DIRECT k=GETBYTE(eaddr);
                                 tw=iareg+k;
   				 SETSTATUS(iareg,k,tw)
   				 iareg=(Byte)tw;break;
   ...
   case 0x9F:  /* STX (STY) direct */ DIRECT
                                  if(!iflag) tw=ixreg; else tw=iyreg;
                                  CLV SETNZ16(tw) SETWORD(eaddr,tw) break;
   case 0xA0: /*SUBA indexed*/  k=GETBYTE(eaddr);
                                 tw=iareg-k;
                                 SETSTATUS(iareg,k,tw)
                                 iareg=(Byte)tw;break;
   case 0xA1: /*CMPA indexed*/  k=GETBYTE(eaddr);
                                 tw=iareg-k;
   				 SETSTATUS(iareg,k,tw) break;
   case 0xA2: /*SBCA indexed*/  k = GETBYTE(eaddr); tw=iareg-k-(iccreg&0x01);
   				 SETSTATUS(iareg,k,tw)
   				 iareg=(Byte)tw;break;
   ...
   case 0xA4: /*ANDA indexed*/  iareg=iareg&GETBYTE(eaddr);SETNZ8(iareg)
   				 CLV break;
   case 0xA5: /*BITA indexed*/  tb=iareg&GETBYTE(eaddr);SETNZ8(tb)
   				 CLV break;
   case 0xA6: /*LDA indexed*/  LOADAC(iareg) CLV SETNZ8(iareg)
                                 break;
   case 0xA7: /*STA indexed */ 
                                 SETNZ8(iareg) CLV STOREAC(iareg) break;
   case 0xA8: /*EORA indexed*/  iareg=iareg^GETBYTE(eaddr);SETNZ8(iareg)
   				 CLV break;
   case 0xA9: /*ADCA indexed*/  k=GETBYTE(eaddr); tw=iareg+k+(iccreg&0x01);
                                 SETSTATUS(iareg,k,tw)
                                 iareg=(Byte)tw;break;   				 
   case 0xAA: /*ORA indexed*/   iareg=iareg|GETBYTE(eaddr);SETNZ8(iareg)
   				 CLV break;
   case 0xAB: /*ADDA indexed*/  k=GETBYTE(eaddr); tw=iareg+k;
   				 SETSTATUS(iareg,k,tw)
   				 iareg=(Byte)tw;break;
   ...
   case 0xAF:  /* STX (STY) indexed */ 
                                  if(!iflag) tw=ixreg; else tw=iyreg;
                                  CLV SETNZ16(tw) SETWORD(eaddr,tw) break;
   case 0xB0: /*SUBA ext*/ EXTENDED k=GETBYTE(eaddr); tw=iareg-k;
                                 SETSTATUS(iareg,k,tw)
                                 iareg=(Byte)tw;break;
   case 0xB1: /*CMPA ext*/ EXTENDED k=GETBYTE(eaddr); tw=iareg-k;
   				 SETSTATUS(iareg,k,tw) break;
   case 0xB2: /*SBCA ext*/ EXTENDED k=GETBYTE(eaddr); tw=iareg-k-(iccreg&0x01);
   				 SETSTATUS(iareg,k,tw)
   				 iareg=(Byte)tw;break;
   ...
   case 0xB4: /*ANDA ext*/ EXTENDED iareg=iareg&GETBYTE(eaddr);SETNZ8(iareg)
   				 CLV break;
   case 0xB5: /*BITA ext*/ EXTENDED tb=iareg&GETBYTE(eaddr);SETNZ8(tb)
   				 CLV break;
   case 0xB6: /*LDA ext*/ EXTENDED LOADAC(iareg) CLV SETNZ8(iareg)
                                 break;
   case 0xB7: /*STA ext */ EXTENDED
                                 SETNZ8(iareg) CLV STOREAC(iareg) break;
   case 0xB8: /*EORA ext*/ EXTENDED iareg=iareg^GETBYTE(eaddr);SETNZ8(iareg)
   				 CLV break;
   case 0xB9: /*ADCA ext*/ EXTENDED k=GETBYTE(eaddr); tw=iareg+k+(iccreg&0x01);
                                 SETSTATUS(iareg,k,tw)
                                 iareg=(Byte)tw;break;   				 
   case 0xBA: /*ORA ext*/  EXTENDED iareg=iareg|GETBYTE(eaddr);SETNZ8(iareg)
   				 CLV break;
   case 0xBB: /*ADDA ext*/ EXTENDED k=GETBYTE(eaddr); tw=iareg+k;
   				 SETSTATUS(iareg,k,tw)
   				 iareg=(Byte)tw;break;
   ...
   case 0xBF:  /* STX (STY) ext */ EXTENDED
                                  if(!iflag) tw=ixreg; else tw=iyreg;
                                  CLV SETNZ16(tw) SETWORD(eaddr,tw) break;
   case 0xC0: /*SUBB immediate*/ IMM8 k=GETBYTE(eaddr); tw=ibreg-k;
                                 SETSTATUS(ibreg,k,tw)
                                 ibreg=(Byte)tw;break;
   case 0xC1: /*CMPB immediate*/ IMM8 k=GETBYTE(eaddr); tw=ibreg-k;
   				 SETSTATUS(ibreg,k,tw) break;
   case 0xC2: /*SBCB immediate*/ IMM8 k=GETBYTE(eaddr); tw=ibreg-k-(iccreg&0x01);
   				 SETSTATUS(ibreg,k,tw)
   				 ibreg=(Byte)tw;break;
   ...
   case 0xC4: /*ANDB immediate*/ IMM8 ibreg=ibreg&GETBYTE(eaddr);
                                 SETNZ8(ibreg)
   				 CLV break;
   case 0xC5: /*BITB immediate*/ IMM8 tb=ibreg&GETBYTE(eaddr);SETNZ8(tb)
   				 CLV break;
   ...
   case 0xC8: /*EORB immediate*/ IMM8 ibreg=ibreg^GETBYTE(eaddr);SETNZ8(ibreg)
   				 CLV break;
   case 0xC9: /*ADCB immediate*/ IMM8 k=GETBYTE(eaddr); tw=ibreg+k+(iccreg&0x01);
                                 SETSTATUS(ibreg,k,tw)
                                 ibreg=(Byte)tw;break;   				 
   case 0xCA: /*ORB immediate*/  IMM8 ibreg=ibreg|GETBYTE(eaddr);SETNZ8(ibreg)
   				 CLV break;
   case 0xCB: /*ADDB immediate*/ IMM8 k=GETBYTE(eaddr); tw=ibreg+k;
   				 SETSTATUS(ibreg,k,tw)
   				 ibreg=(Byte)tw;break;
   ...
   case 0xCF:  /* STU (STS) immediate (orthogonality) */ IMM16
                                  ...
                                  CLV SETNZ16(tw) SETWORD(eaddr,tw) break;
   case 0xD0: /*SUBB direct*/ DIRECT k=GETBYTE(eaddr); tw=ibreg-k;
                                 SETSTATUS(ibreg,k,tw)
                                 ibreg=(Byte)tw;break;
   case 0xD1: /*CMPB direct*/ DIRECT k=GETBYTE(eaddr); tw=ibreg-k;
   				 SETSTATUS(ibreg,k,tw) break;
   case 0xD2: /*SBCB direct*/ DIRECT k=GETBYTE(eaddr); tw=ibreg-k-(iccreg&0x01);
   				 SETSTATUS(ibreg,k,tw)
   				 ibreg=(Byte)tw;break;
   ...
   case 0xD4: /*ANDB direct*/ DIRECT ibreg=ibreg&GETBYTE(eaddr);SETNZ8(ibreg)
   				 CLV break;
   case 0xD5: /*BITB direct*/ DIRECT tb=ibreg&GETBYTE(eaddr);SETNZ8(tb)
   				 CLV break;
   case 0xD6: /*LDB direct*/ DIRECT LOADAC(ibreg) CLV SETNZ8(ibreg)
                                 break;
   case 0xD7: /*STB direct  */ DIRECT
                                 SETNZ8(ibreg) CLV STOREAC(ibreg) break;
   case 0xD8: /*EORB direct*/ DIRECT ibreg=ibreg^GETBYTE(eaddr);SETNZ8(ibreg)
   				 CLV break;
   case 0xD9: /*ADCB direct*/ DIRECT k=GETBYTE(eaddr); tw=ibreg+k+(iccreg&0x01);
                                 SETSTATUS(ibreg,k,tw)
                                 ibreg=(Byte)tw;break;   				 
   case 0xDA: /*ORB direct*/  DIRECT ibreg=ibreg|GETBYTE(eaddr);SETNZ8(ibreg)
   				 CLV break;
   case 0xDB: /*ADDB direct*/ DIRECT k=GETBYTE(eaddr); tw=ibreg+k;
   				 SETSTATUS(ibreg,k,tw)
   				 ibreg=(Byte)tw;break;
   ...
   case 0xDF:  /* STU (STS) direct  */ DIRECT
                                  if(!iflag) tw=iureg; else tw=isreg;
                                  CLV SETNZ16(tw) SETWORD(eaddr,tw) break;
   case 0xE0: /*SUBB indexed*/  k=GETBYTE(eaddr); tw=ibreg-k;
                                 SETSTATUS(ibreg,k,tw)
                                 ibreg=(Byte)tw;break;
   case 0xE1: /*CMPB indexed*/  k=GETBYTE(eaddr); tw=ibreg-k;
   				 SETSTATUS(ibreg,k,tw) break;
   case 0xE2: /*SBCB indexed*/  k=GETBYTE(eaddr);
   ...
   case 0xE4: /*ANDB indexed*/  ibreg=ibreg&GETBYTE(eaddr);SETNZ8(ibreg)
   				 CLV break;
   case 0xE5: /*BITB indexed*/  tb=ibreg&GETBYTE(eaddr);SETNZ8(tb)
   				 CLV break;
   case 0xE6: /*LDB indexed*/  LOADAC(ibreg) CLV SETNZ8(ibreg)
                                 break;
   case 0xE7: /*STB indexed  */ 
                                 SETNZ8(ibreg) CLV STOREAC(ibreg) break;
   case 0xE8: /*EORB indexed*/  ibreg=ibreg^GETBYTE(eaddr);SETNZ8(ibreg)
   				 CLV break;
   case 0xE9: /*ADCB indexed*/  k=GETBYTE(eaddr); tw=ibreg+k+(iccreg&0x01);
                                 SETSTATUS(ibreg,k,tw)
                                 ibreg=(Byte)tw;break;   				 
   case 0xEA: /*ORB indexed*/   ibreg=ibreg|GETBYTE(eaddr);SETNZ8(ibreg)
   				 CLV break;
   case 0xEB: /*ADDB indexed*/   k = GETBYTE(eaddr); tw=ibreg+k;
   				 SETSTATUS(ibreg,k,tw)
   				 ibreg=(Byte)tw;break;
   ...
   case 0xEF:  /* STU (STS) indexed  */ 
                                  if(!iflag) tw=iureg; else tw=isreg;
                                  CLV SETNZ16(tw) SETWORD(eaddr,tw) break;
   case 0xF0: /*SUBB ext*/ EXTENDED k=GETBYTE(eaddr); tw=ibreg-k;
                                 SETSTATUS(ibreg,k,tw)
                                 ibreg=(Byte)tw;break;
   case 0xF1: /*CMPB ext*/ EXTENDED k=GETBYTE(eaddr); tw=ibreg-k;
   				 SETSTATUS(ibreg,k,tw) break;
   case 0xF2: /*SBCB ext*/ EXTENDED k=GETBYTE(eaddr); tw=ibreg-k-(iccreg&0x01);
   				 SETSTATUS(ibreg,k,tw)
   				 ibreg=(Byte)tw;break;
   ...
   case 0xF4: /*ANDB ext*/ EXTENDED ibreg=ibreg&GETBYTE(eaddr);SETNZ8(ibreg)
   				 CLV break;
   case 0xF5: /*BITB ext*/ EXTENDED tb=ibreg&GETBYTE(eaddr);SETNZ8(tb)
   				 CLV break;
   case 0xF6: /*LDB ext*/ EXTENDED LOADAC(ibreg) CLV SETNZ8(ibreg)
                                 break;
   case 0xF7: /*STB ext  */ EXTENDED
                                 SETNZ8(ibreg) CLV STOREAC(ibreg) break;
   case 0xF8: /*EORB ext*/ EXTENDED ibreg=ibreg^GETBYTE(eaddr);SETNZ8(ibreg)
   				 CLV break;
   case 0xF9: /*ADCB ext*/ EXTENDED k=GETBYTE(eaddr); tw=ibreg+k+(iccreg&0x01);
                                 SETSTATUS(ibreg,k,tw)
                                 ibreg=(Byte)tw;break;   				 
   case 0xFA: /*ORB ext*/  EXTENDED ibreg=ibreg|GETBYTE(eaddr);SETNZ8(ibreg)
   				 CLV break;
   case 0xFB: /*ADDB ext*/ EXTENDED k=GETBYTE(eaddr); tw=ibreg+k;
   				 SETSTATUS(ibreg,k,tw)
   				 ibreg=(Byte)tw;break;
