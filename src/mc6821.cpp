...
{
	switch (offset & 0x03) {
	case 0:	if(cra & 0x04) {		// check data direction bit
				...
				return ddra;
	case 1:	requestInputA();		// request for input
		return cra;
	case 2:	if(crb & 0x04)	{		// check data direction bit
				...
				return ddrb;
	case 3:	requestInputB();		// request for input
		return cra;
	}
	return 0;
}
...
{
	switch (offset & 0x03) {
	case 0:	if(BTST2(cra)) {	// check data direction bit
			ora = val;
	...
	case 1:	cra = val;
		...
		break;
	case 2:	if(BTST2(crb)) {	// check data direction bit
			orb = val;
...
void Mc6821::writeOutputA(Byte val)
{
}
...
void Mc6821::writeOutputB(Byte val)
{
}
...
{
	switch (control_line) {
	case CA1:	BSET7(cra);		// set IRQA1 flag
			if (BTST0(cra)) {
	...
	case CA2:	if (BTST5(cra)) {
			...
			break;
	case CB1:	BSET7(crb);		// set IRQB1 flag
			if (BTST0(crb)) {
