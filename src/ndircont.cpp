...
BString NafsDirectoryContainer::unix_filename(SWord file_id) const
{
	if (file_id < 0)
	{
		...
		return get_unix_filename(
		   (const char *)&((pflex_directory + (file_id/10))->dir_entry[file_id%10].filename));
} // unix_filename

// return the record nr (beginning from 0) of a new file which first
// sector has index 'index' into flex link table
Word NafsDirectoryContainer::record_nr_of_new_file(SWord new_file_index, Word index) const
{
	SWord   i;
	Word    record_nr;
...
bool NafsDirectoryContainer::is_in_file_random(const char *ppath, const char *pfilename)
{
	char    str[PATH_MAX+1];
	...
	if ((pd = opendir(*dir)) != NULL) {
		while ((pentry = readdir(pd)) != NULL) {
			...
			if (IsFlexFilename(pfilename, (char *)&name, (char *)&ext) &&
			   ...
			   strcmp(pfilename, RANDOM_FILE_LIST) && // exclude file "random"
			   sbuf.st_size > 0 &&
...
void NafsDirectoryContainer::change_file_id(SWord index, SWord old_id, SWord new_id) const
{
	SWord i = index;
...
void NafsDirectoryContainer::check_for_delete(SWord dir_index, s_dir_sector *pdb) const
{
	Word            i;
	SWord           index;
	char            path[PATH_MAX+1];
...
void NafsDirectoryContainer::check_for_rename(SWord dir_index, s_dir_sector *pdb) const
{
	Word    i;
	...
	for (i = 0; i < 10; i++) {
		...
		pfilename2 = get_unix_filename(
			(const char *)pdb->dir_entry[i].filename);
...
SWord NafsDirectoryContainer::set_file_time(char *ppath, Byte month, Byte day, Byte year) const
{
	struct stat    statbuf;
...
Byte NafsDirectoryContainer::check_for_new_file(SWord dir_index, s_dir_sector *pd) const
{
	Word            i, j;
	SWord           index;
	char            old_path[PATH_MAX+1], new_path[PATH_MAX+1];
