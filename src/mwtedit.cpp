...
void MemoryWindowTextEdit::keyPressEvent(QKeyEvent *event)
{
    ...
    const auto newText = toUpperCase ? event->text().toUpper() : event->text();
    QKeyEvent newEvent(QEvent::KeyPress, event->key(), modifier, newText);

    UpdateEventType(newEvent.type());
    const auto text = newEvent.text();
    // Check for currently valid character (no Ctrl keys).
    const bool isValidChar = (text.size() == 1 &&
            validCharacters.contains(text.at(0).toLatin1()));

    switch (newEvent.modifiers())
    {
        case Qt::NoModifier:
        case Qt::KeypadModifier:
        case Qt::ControlModifier | Qt::AltModifier:
        case Qt::GroupSwitchModifier:
            if (isValidChar || supportedKeys.contains(newEvent.key()))
            {
...
// This input method is used on Linux to process dead keys.
